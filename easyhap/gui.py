...
class EasyHapGUI(tk.Tk):
    ...
    def _build(self):
        ...
        self._file_row(inp,4,"Sample-group file",self.group,False)
        ...
        self.ld=tk.BooleanVar(value=True); self.processed=tk.BooleanVar(value=True)
    ...
    def _run(self):
        ...
        kwargs=dict(vcf_path=self.vcf.get().strip(),group_file=self.group.get().strip() or None,outdir=out,region=self.region.get().strip() or None,region_file=self.region_file.get().strip() or None,
                    mode=self.mode.get(),hetero_policy=self.hetero.get(),trait_file=self.traits.get().strip() or None,gff_file=self.gff.get().strip() or None,
        ...
        self.status.set("Running…"); self._log("Running EasyHap 1.1.0…")
        def worker():
            try:
